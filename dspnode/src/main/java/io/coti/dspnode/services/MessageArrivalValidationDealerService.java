...
import io.coti.dspnode.data.DealerData;
...
public class MessageArrivalValidationDealerService {
    ...
    public void validatedMessagesAcceptedByRecipient(){
        log.info("DSP node Starting arrival validation flow for sent messages.");
    ...
    private List<MessageArrivalValidationData> signAndSend(MessageArrivalValidationData data) {
        ...
        //TODO 4/27/2019 astolia: need to filter different messages to different nodes for future generic usgae?
        return networkService.sendDataToConnectedNodeByHttp(data);
    }

    private void handleMessageValidationResponses(List<MessageArrivalValidationData> responses, MessageArrivalValidationData data){
        List<MessageArrivalValidationData> validateResponses = validateMessages(responses);
        if(validateResponses.isEmpty()){
            log.warn("No valid response received");
            return;
        }
        validateResponses.forEach(this::printUnreceivedHashes);
        removeVerifiedHashesFromDB(data, validateResponses);
    }
