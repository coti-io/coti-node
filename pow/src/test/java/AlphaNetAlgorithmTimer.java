import coti.crypto.AlphaNetAlgorithm;
...
class AlphaNetAlgorithmTimer {
    ...
    @Test
    void skein() {
        IAlgorithm.AlgorithmTypes type = IAlgorithm.AlgorithmTypes.Skein512;
    ...
    @Test
    void blake() {
        IAlgorithm.AlgorithmTypes type = IAlgorithm.AlgorithmTypes.BLAKE512;
        results.put(type, dowork(type));
    }

    @Test
    void sha() {
        IAlgorithm.AlgorithmTypes type = IAlgorithm.AlgorithmTypes.SHA512;
        results.put(type, dowork(type));
    }

    @Test
    void keccak() {
        IAlgorithm.AlgorithmTypes type = IAlgorithm.AlgorithmTypes.Keccak512;
        results.put(type, dowork(type));
    }
    ...
    private double dowork(IAlgorithm.AlgorithmTypes type) {
        ...
        AlphaNetAlgorithm algo = new AlphaNetAlgorithm(type);
        long startTimeRound = AlphaNetBenchmarkBatching.getCpuTime();

        for (int i = 0; i < sampleSize; i++) {
            algo.hash(inputData[i]);
        }

        long endTimeRound = AlphaNetBenchmarkBatching.getCpuTime();
