...
public class CurrencyCrypto extends SignatureCrypto<CurrencyData> {
    ...
    public byte[] getSignatureMessage(CurrencyData currencyData) {
        ...
        byte[] typeInBytes = currencyData.getCurrencyTypeData().getCurrencyType().toString().getBytes();
        ...
        ByteBuffer buffer = ByteBuffer.allocate(hashInBytes.length + nameInBytes.length + symbolInBytes.length +
                currencyTypeInBytes.length + descriptionInBytes.length + totalSupplyInBytes.length + Integer.BYTES + Long.BYTES +
                typeInBytes.length + originatorHashInBytes.length + registrarHashInBytes.length);
        buffer.put(hashInBytes).put(nameInBytes).put(symbolInBytes).put(currencyTypeInBytes).put(descriptionInBytes).put(totalSupplyInBytes).
