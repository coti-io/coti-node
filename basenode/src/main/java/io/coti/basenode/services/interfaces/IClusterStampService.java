package io.coti.basenode.services.interfaces;


import io.coti.basenode.http.GetClusterStampFileNames;
import org.springframework.http.ResponseEntity;

/**
 * An interface that defines basic Cluster Stamp functionality for all nodes that take part in cluster stamp flow.
 */
public interface IClusterStampService {

    void loadClusterStamp();

    ResponseEntity<GetClusterStampFileNames> getRequiredClusterStampNames(GetClusterStampFileNames getClusterStampFileNames);

    void getClusterStampFromBackupNode();

}