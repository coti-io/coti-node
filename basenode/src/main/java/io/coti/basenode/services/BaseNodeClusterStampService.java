            ...
            while ((line = bufferedReader.readLine()) != null) {
                ...
                if (line.isEmpty()) {
                    if (relevantLineNumber.get() < NUMBER_OF_GENESIS_ADDRESSES_MIN_LINES) {
                        throw new ClusterStampValidationException(String.format("Clusterstamp file has less than necessary number of balance lines"));
                    } else {
                        if (!finishedBalances)
                            finishedBalances = true;
                        else
                            throw new ClusterStampValidationException(String.format("Unnecessary empty line at clusterstamp file."));
                    }
                } else {
                    if (!finishedBalances) {
                        ...
                        if (!reachedSignatureSection) {
                            if (!line.contentEquals(SIGNATURE_LINE_TOKEN))
                                throw new ClusterStampValidationException(String.format("Invalid signature line notification at clusterstamp file"));
                            else
