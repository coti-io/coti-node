    ...
    private List<TransactionData> getNeighbourSources(
        ...
        try {
            ...
            if (numberOfSources > 0) {
                ...
                neighbourSources.addAll(trustScoreToSourceListMapping.get(roundedTrustScore).stream().map(hash -> sourceMap.get(hash)).collect(toList()));

                for (int trustScoreDifference = 0; trustScoreDifference < maxNeighbourhoodRadius; trustScoreDifference++) {
                    if (lowIndex >= 0) {
                        neighbourSources.addAll(trustScoreToSourceListMapping.get(lowIndex).stream().map(hash -> sourceMap.get(hash)).collect(toList()));
                    }
                    if (highIndex <= 100) {
                        neighbourSources.addAll(trustScoreToSourceListMapping.get(highIndex).stream().map(hash -> sourceMap.get(hash)).collect(toList()));
                    }
